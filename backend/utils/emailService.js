...
const initializeTransporter = () => {
  ...
  // Verify SMTP connection (async, non-blocking)
...
export const sendPasswordChangeConfirmation = async (email, name) => {
  ...
  if (!mailTransporter || !isConfigured) {
    console.warn('⚠️  Password change confirmation email skipped - email service not configured');
    return null; // Don't throw error, just skip
  }
  ...
  try {
    ...
    console.error('   This will not block password reset');
    return null;
  }
};
